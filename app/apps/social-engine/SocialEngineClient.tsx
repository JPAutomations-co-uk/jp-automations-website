...
import {
  ...
  Layers, Sparkles, Globe, MessageSquare, AtSign
} from 'lucide-react';
...
// --- DATA ---
const contentTypes = [
  { name: "Instagram Reels", tokens: "15", desc: "Script, storyboard, and generate short-form video reels tailored to your niche and brand style.", icon: Film },
  { name: "Carousels & Images", tokens: "5–10", desc: "7-slide carousels and branded AI images in any aspect ratio — square, portrait, story, or landscape.", icon: Layers },
  { name: "LinkedIn Posts", tokens: "3", desc: "High-performing posts with hooks optimised for the 210-character cutoff and strong CTAs.", icon: MessageSquare },
  { name: "LinkedIn Images", tokens: "5", desc: "On-brand images for LinkedIn — aspect ratios optimised for the feed.", icon: Image },
  { name: "Tweets & Threads", tokens: "2–8", desc: "Single tweets and multi-tweet threads with tone, goal, and hook control.", icon: AtSign },
  { name: "Account Strategy", tokens: "5", desc: "Tiered engagement targets — reply targets, peer accounts, and emerging creators.", icon: BarChart3 },
  { name: "Outlier Detection", tokens: "10", desc: "Find viral YouTube videos in your niche with performance analysis and scoring.", icon: Search },
  { name: "Title Variants", tokens: "3", desc: "Generate title variations from proven outlier patterns directly into your sheet.", icon: Type },
  { name: "Content Plans", tokens: "25", desc: "A full month of planned content for any platform — topics, formats, posting schedule, and funnel strategy.", icon: CalendarDays },
  { name: "Ad Creatives", tokens: "8–15", desc: "Meta-ready ad images and copy for paid campaigns — single images or carousels.", icon: Megaphone },
];
...
const testimonials = [
  ...
  { name: "Marcus T.", role: "Fitness Coach", text: "The LinkedIn posts genuinely outperform what I was writing myself. Better hooks, better CTAs, and a fraction of the time.", avatar: "MT" },
  ...
  { name: "David L.", role: "Accountant", text: "I never used X before. Now I\u2019m posting 3 threads a week and getting inbound leads from each one.", avatar: "DL" },
  ...
  { name: "James H.", role: "Roofing Contractor", text: "The YouTube outlier scanner found 4 trending topics I\u2019d never have spotted. Game-changer for planning my content.", avatar: "JH" },
];
...
const faqs = [
  ...
  { q: "Which platforms are supported?", a: "Instagram, LinkedIn, X (Twitter), and YouTube. Each platform has dedicated tools optimised for its format and algorithm." },
  ...
  { q: "Do tokens expire?", a: "No. Tokens never expire and work across all platforms — Instagram, LinkedIn, X, YouTube, SEO Blog, and every new app we release." },
  { q: "How do I get access?", a: "Every engine is custom-built for your business. Book a 30-minute discovery call and we\u2019ll walk you through how it works, set up your brand profile, and get you generating content the same week." },
];
...
const Navbar = ({ startAnim }: { startAnim: boolean }) => (
  ...
  >
    <motion.a href="/" whileHover={{ scale: 1.05 }} className="font-bold text-lg tracking-tight cursor-pointer flex items-center gap-2">
      ...
      <span className="text-white">Social<span className="text-teal-400">Engine</span></span>
    </motion.a>
  ...
  <section className="pt-32 pb-20 px-4 max-w-5xl mx-auto text-center relative">
    ...
    <motion.div variants={staggerContainer} initial="initial" animate={startAnim ? "whileInView" : "initial"} className="flex flex-col items-center relative z-10">
      ...
      >
        ...
        <span className="text-sm font-medium text-gray-300">Private Access Only</span>
      </motion.div>
      ...
      <motion.p variants={staggerItem} className="text-xl text-gray-400 max-w-2xl mx-auto mb-12 leading-relaxed">
        Every engine is custom-built around your business, your brand, and your audience. AI-powered content for Instagram, LinkedIn, X, and YouTube — trained on your voice, not a template.
      </motion.p>
      ...
      <motion.div variants={staggerItem} className="flex flex-col items-center gap-6 w-full max-w-md mx-auto">
        ...
        >
          Book a Discovery Call <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
        </motion.a>
        ...
        <div className="flex items-center gap-6 text-xs font-semibold text-gray-500">
          <motion.span whileHover={{ color: '#2dd4bf' }} className="flex items-center gap-1.5 cursor-default"><Sparkles className="w-4 h-4 text-teal-400" /> 4 PLATFORMS</motion.span>
          <motion.span whileHover={{ color: '#2dd4bf' }} className="flex items-center gap-1.5 cursor-default"><Zap className="w-4 h-4 text-teal-400" /> BUILT FOR YOU</motion.span>
        </div>
      </motion.div>
    </motion.div>
  </section>
        ...
        <motion.div animate={{ x: [0, -1200] }} transition={{ repeat: Infinity, repeatType: "loop", duration: 30, ease: "linear" }} className="flex gap-12 text-gray-500 font-semibold tracking-widest text-sm pl-12 pr-12">
          {[...Array(8)].map((_, i) => (
            <React.Fragment key={i}>
              <span className="flex items-center gap-2"><Film className="w-4 h-4" /> INSTAGRAM REELS</span>
              <span className="flex items-center gap-2"><MessageSquare className="w-4 h-4" /> LINKEDIN POSTS</span>
              <span className="flex items-center gap-2"><AtSign className="w-4 h-4" /> X THREADS</span>
              <span className="flex items-center gap-2"><Search className="w-4 h-4" /> YOUTUBE OUTLIERS</span>
            </React.Fragment>
          ))}
        </motion.div>
      </div>
        ...
        <motion.div animate={{ x: [-1200, 0] }} transition={{ repeat: Infinity, repeatType: "loop", duration: 30, ease: "linear" }} className="flex gap-12 text-teal-400/60 font-semibold tracking-widest text-sm pl-12 pr-12">
          {[...Array(8)].map((_, i) => (
            <React.Fragment key={i}>
              ...
              <span className="flex items-center gap-2"><Type className="w-4 h-4" /> TITLE VARIANTS</span>
            </React.Fragment>
          ))}
        </motion.div>
      </div>
  ...
  <section className="py-24 px-4 max-w-6xl mx-auto">
      ...
      <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: "-100px" }} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {[
            ...
            { icon: BarChart3, val: <><AnimatedNumber value={4} /></>, label: "Platforms, One Balance" },
        ...
        >
          ...
          <div className="space-y-6 flex-grow">
            {[
              { name: "LinkedIn Post", cost: "3 tokens" },
              { name: "Single Image", cost: "5 tokens" },
              { name: "X Thread", cost: "8 tokens" },
    ...
    <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: "-100px" }} className="mb-32">
      <motion.h2 variants={staggerItem} className="text-3xl md:text-5xl font-bold tracking-tight text-white mb-4 text-center">
        Everything you need across <strong>every platform.</strong>
      </motion.h2>
      <motion.p variants={staggerItem} className="text-gray-400 text-center mb-16 max-w-xl mx-auto">
        Four platforms. One token balance. Generate what you need, when you need it.
      </motion.p>
    ...
    <section className="py-24 px-4 max-w-6xl mx-auto">
      ...
      <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: "-100px" }} className="mb-32">
        <motion.div variants={staggerItem} className="text-center mb-16">
          ...
          <p className="text-gray-400 max-w-xl mx-auto">Pay only for what you generate. Tokens never expire and work across all apps. Pricing discussed on your discovery call.</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
          <GlowCard className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-8 hover:border-white/10">
            <motion.div variants={staggerItem}>
              ...
              <motion.a href="/book-call" whileHover={{ scale: 1.03, backgroundColor: 'rgba(255,255,255,0.06)' }} whileTap={{ scale: 0.97 }} className="w-full py-4 rounded-xl border border-white/10 text-white font-semibold transition-colors block text-center">Book a Call</motion.a>
            </motion.div>
          </GlowCard>
          ...
          >
            ...
            <ul className="mb-8 space-y-3 text-gray-300 text-sm">
              {["350 tokens (35 carousels, 116 LinkedIn posts, or 23 reels)", "Works across all apps", "Tokens never expire", "Best per-token rate"].map((f, i) => (
                <motion.li key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-teal-400" /> {f}</motion.li>
              ))}
            </ul>
            <motion.a href="/book-call" whileHover={{ scale: 1.03, boxShadow: '0 0 30px rgba(45,212,191,0.3)' }} whileTap={{ scale: 0.97 }} className="w-full py-5 rounded-xl bg-teal-400 text-black font-bold hover:bg-teal-300 transition-colors text-lg block text-center">Book a Discovery Call</motion.a>
          </motion.div>
      ...
      <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: "-100px" }} className="mb-32">
        ...
        <div className="flex flex-col md:flex-row justify-between relative max-w-4xl mx-auto">
          ...
          />
          {[
            { step: "01", label: "Book a discovery call", desc: "We learn your business, your brand, and your goals in a 30-minute call." },
            { step: "02", label: "We build your engine", desc: "Your agent is custom-configured with your tone, style, and audience across all platforms." },
            { step: "03", label: "Generate and post", desc: "Start creating content in seconds — reels, carousels, posts, and plans, all on-brand." },
          ].map((item, i) => (
            <motion.div
              key={i}
              variants={staggerItem}
      ...
      >
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-400 mb-8 leading-relaxed max-w-xl mx-auto">Every engine is custom-built around your business. Book a 30-minute discovery call and we&apos;ll map out exactly how it works for you.</p>
            <motion.a
              href="/book-call"
              whileHover={{ scale: 1.05, boxShadow: '0 0 50px rgba(45,212,191,0.35)' }}
              whileTap={{ scale: 0.97 }}
            ...
            >
              Book a Discovery Call <ArrowRight className="w-5 h-5 inline ml-2" />
            </motion.a>
          </div>
        </motion.div>
      </motion.div>
        ...
        <footer className="border-t border-white/[0.06] py-10 px-6 relative z-10">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <motion.a href="/" whileHover={{ scale: 1.05 }} className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="font-bold text-white text-sm">Social<span className="text-teal-400">Engine</span></span>
            </motion.a>
