...
export default function AdCreativesPage() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!user) {
      router.push("/login?redirect=/dashboard/instagram/ads")
      return
    }
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            {user ? (
              ...
              <a href="/login?redirect=/dashboard/instagram/ads" className="px-5 py-2.5 text-sm font-semibold bg-teal-400 text-black rounded-lg hover:bg-teal-300 transition-all hover:scale-105">
                Sign In
              </a>
            )}
          </div>
      ...
      <div className={`fixed inset-0 bg-[#050505]/95 backdrop-blur-xl z-40 flex items-center justify-center transition-all duration-500 ${isMobileMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className="flex flex-col items-center gap-8 text-center">
          ...
          {user ? (
            ...
            <a href="/login?redirect=/dashboard/instagram/ads" className="text-4xl font-bold text-teal-400 hover:text-teal-300 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Sign In</a>
          )}
        </div>
      </div>
