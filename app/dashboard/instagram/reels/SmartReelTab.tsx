...
export default function SmartReelTab({ uploadAsset, handleDownload }: SmartReelTabProps) {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (!user) {
      router.push("/login?redirect=/dashboard/instagram/reels")
      return
    }
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      if (brief.inputMode === "own_images") {
        // Convert brief to luxury reel frame plan format
        const framePlan = {
          reel_concept: {
            title: brief.topic,
        ...
        // AI images — Flux Pro via existing reel endpoint
            ...
            <div key={label} className="flex items-center gap-2 flex-1">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                  isActive ? "bg-teal-400 text-black" : isDone ? "bg-teal-400/20 text-teal-400" : "bg-white/[0.06] text-gray-600"
                }`}
              >
                {isDone ? "\u2713" : i + 1}
              </div>
              <span className={`text-xs font-medium hidden sm:block ${isActive ? "text-white" : isDone ? "text-teal-400/60" : "text-gray-600"}`}>
                {label}
              </span>
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-1 p-1 bg-white/[0.03] border border-white/[0.08] rounded-xl">
            <button
              ...
              className={`flex-1 py-2.5 px-4 text-sm font-semibold rounded-lg transition-all ${
                inputMode === "own_images" ? "bg-teal-400/10 text-teal-400 border border-teal-400/20" : "text-gray-500 hover:text-gray-300"
              }`}
            >
              My Images
            </button>
            <button
              ...
              className={`flex-1 py-2.5 px-4 text-sm font-semibold rounded-lg transition-all ${
                inputMode === "ai_images" ? "bg-teal-400/10 text-teal-400 border border-teal-400/20" : "text-gray-500 hover:text-gray-300"
              }`}
            >
              AI-Generated
            </button>
          ...
          <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6">
            ...
            <textarea
              ...
              placeholder={
                inputMode === "own_images"
                  ? "e.g. 5 reasons to redesign your office..."
                  : "e.g. Modern kitchen renovation ideas for 2025..."
              }
            />
          </div>
        ...
        <div className="space-y-6">
                  ...
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform ${
                        answers[q.id] === "true" ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                )}
          ...
          <div className="flex gap-3">
            ...
            >
              {generatingBrief ? (
                <>
                  <Spinner /> Building creative brief...
                </>
              ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-5">
            ...
            <div className="flex flex-wrap gap-2 text-xs text-gray-500">
              <span>
                Topic: <span className="text-gray-300">{brief.topic}</span>
              </span>
              <span>&middot;</span>
              <span>
                Template: <span className="text-gray-300">{TEMPLATE_LIST.find((t) => t.id === brief.template)?.name || brief.template}</span>
              </span>
              <span>&middot;</span>
              <span>
                Mood: <span className="text-gray-300">{brief.mood}</span>
              </span>
              <span>&middot;</span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      {slide.camera_movement && <span className="text-[11px] text-gray-600">{slide.camera_movement}</span>}
                    </div>
                    ...
                    {slide.source_image && <p className="text-[11px] text-gray-600 mt-1">Source: {slide.source_image}</p>}
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 ${
                brief.inputMode === "own_images" ? "bg-amber-400 text-black hover:bg-amber-300" : "bg-teal-400 text-black hover:bg-teal-300"
              }`}
            >
              {generating ? (
                <>
                  <Spinner className="w-4 h-4" /> Starting generation...
                </>
              ) : (
                `Approve & Generate (${tokenCost} tokens)`
              )}
            </button>
          </div>
            ...
            <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 space-y-4">
              ...
              <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.max(0, Math.min(100, luxuryJob.progress))}%` }}
                />
              </div>
              ...
              {luxuryJob.outputUrl && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      onClick={() => handleDownload(luxuryJob.outputUrl!, `smart-reel-${luxuryJob.jobId}.mp4`)}
