  ...
  try {
    ...
    if (AUDIENCE_ID) {
      resend.contacts.create({
        ...
        email,
        unsubscribed: false,
      }).catch((err) => console.error('Failed to add waitlist contact to audience:', err))
    }
    ...
    const { error: resendError } = await resend.emails.send({
      ...
      to: email,
      subject: "You're on the waitlist — JP Automations Apps",
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 24px; color: #111;">
          ...
          <p style="font-size: 15px; color: #64748b; line-height: 1.7; margin-bottom: 32px;">
            We're opening early access to our AI-powered content apps soon. You'll be the first to know when it's ready — no action needed on your end.
          </p>
          ...
          <div style="border-top: 1px solid #f1f5f9; padding-top: 24px; margin-top: 8px;">
            <p style="font-size: 13px; color: #94a3b8; line-height: 1.6; margin: 0;">
              Want your business automated end to end? Book a call at
    ...
    if (resendError) {
      console.error('Waitlist email error:', resendError)
