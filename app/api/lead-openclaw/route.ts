...
export async function POST(request: NextRequest) {
  try {
    ...
    addToLoops({ email, firstName, source: 'openclaw-guide' })
    ...
    const { error: resendError } = await resend.emails.send({
      ...
      to: email,
      subject: 'The OpenClaw Security Guide',
      text: `The OpenClaw Security Guide is ready. It covers exactly how to configure OpenClaw safely — so you're not putting your data or your clients' data at risk.

Read it here: jpautomations.co.uk/openclaw-guide

JP

P.S. Start with the permissions section. Most people give OpenClaw far more access than it actually needs — and that's where the risk comes from.`,
    })
    ...
    const shouldNurture = await tryEnrollSequence(email, 'openclaw')
...
P.S. If you're using AI tools like OpenClaw for your business already, the Prompt Engineering Guide will make everything you do with them significantly better. Covers the structure behind every prompt I write. jpautomations.co.uk/free-prompt-guide`,
...
P.S. And if you'd rather keep building it yourself, the automation blueprint covers the full framework for deciding what's worth automating and what's not. jpautomations.co.uk/free-blueprint`,
    }).catch((err) => console.error('Nurture email 3 error:', err))
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Lead openclaw email error:', error)
