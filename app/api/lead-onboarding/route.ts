...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: resendError } = await resend.emails.send({
      ...
      to: email,
      subject: 'Your 10-Minute Onboarding System',
      text: `The 10-Minute Onboarding System is ready — step-by-step instructions, the exact tools, and copy-paste prompts for everything. Setup takes 2-3 hours once. Every client after that is under 10 minutes.

Open it here: jpautomations.co.uk/onboarding-guide

JP

P.S. Start with the intake form questions — those are the foundation everything else plugs into. Get those right and the rest practically builds itself.`,
    })
...
P.S. If the bottleneck is actually proposals and contracts — formatting documents, adjusting scope, getting everything client-ready — this is probably more useful right now: jpautomations.co.uk/free-client-folder. One brief per client, everything generated in 5 minutes.`,
...
P.S. And if you'd rather keep building it yourself, the automation blueprint is probably the most useful next step — the full framework for deciding what to build and in what order. jpautomations.co.uk/free-blueprint`,
    ...
    console.error('Lead onboarding email error:', error)
