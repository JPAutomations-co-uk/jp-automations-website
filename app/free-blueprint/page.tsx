    ...
    <main className="relative min-h-screen bg-[#0B0F14] flex items-center justify-center overflow-hidden px-6 py-24">
      ...
      <div className="relative z-10 w-full max-w-xl text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold text-white leading-[1.1] mb-5">
          The Automation<br />
          <span className="text-teal-400">Blueprint</span>
        </h1>

        {/* Subheadline */}
        <p className="text-gray-400 text-[17px] leading-[1.7] mb-10 max-w-md mx-auto">
          The exact framework I use to find, build, and scale the right automation for any business — without touching a single tool until the problem is fully understood.
        </p>

        {/* What's inside */}
        <div className="text-left bg-white/[0.03] border border-white/[0.08] rounded-2xl px-6 py-5 mb-10 space-y-3">
          <p className="text-white text-sm font-semibold mb-1">What's inside:</p>
          {[
            "The 30-minute Automation Audit — map any business's systems fast",
            'The Reddit Research Method — find real pain points in any niche',
            'The Two Rules — how to know if an automation is worth building',
            'Workflows that always work — 5 proven automations for service businesses',
            'A decision tree — exactly where to start, every time',
          ].map((item) => (
        ...
        {status === 'success' ? (
          <div className="bg-teal-400/10 border border-teal-400/30 rounded-2xl px-8 py-8">
            ...
            <p className="text-gray-400 text-sm">
              Check your inbox — the blueprint is heading to <span className="text-white">{email}</span> now.
            </p>
          </div>
        ) : (
