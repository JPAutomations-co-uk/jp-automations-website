...
export default function FreeClientFolderPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/lead-client-folder', {
        method: 'POST',
    ...
    <main className="relative min-h-screen bg-[#0B0F14] flex items-center justify-center overflow-hidden px-6 py-24">
      ...
      <div className="relative z-10 w-full max-w-xl text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold text-white leading-[1.1] mb-5">
          The AI<br />
          <span className="text-teal-400">Client Folder</span>
        </h1>

        <p className="text-gray-400 text-[17px] leading-[1.7] mb-3 max-w-md mx-auto">
          Fill in one brief per client. AI generates your proposal, invoice, contract, and CRM entry — in under 5 minutes.
        </p>

        <p className="text-gray-500 text-sm mb-8 max-w-sm mx-auto">
          Setup takes 30 minutes once. Every deal after that takes under 5 minutes.
        </p>

        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl px-6 py-5 mb-8 text-left">
          <p className="text-white text-sm font-semibold mb-3">What&apos;s inside:</p>
          <div className="space-y-3">
            {[
              'The client brief template — fill it in once, use it for everything',
              'Proposal prompt — generates a client-ready proposal from your brief',
              'Invoice prompt — line items, VAT, payment terms, all formatted',
              'Contract prompt — plain-English service agreement, UK-ready',
              'CRM prompt — structured Notion entry from your brief in one paste',
              'Exact folder structure to keep every client organised',
            ].map((item) => (
        ...
        {status === 'success' ? (
          <div className="bg-teal-400/10 border border-teal-400/30 rounded-2xl px-8 py-8">
            ...
            <p className="text-gray-400 text-sm">
              Check your inbox — the folder system is heading to <span className="text-white">{email}</span> now.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {status === 'loading' ? 'Sending...' : 'Send Me the Folder →'}
            </button>
            {status === 'error' && (
