...
export default function FreePromptPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/lead-prompt', {
        method: 'POST',
    ...
    <main className="relative min-h-screen bg-[#0B0F14] flex items-center justify-center overflow-hidden px-6 py-24">
      ...
      <div className="relative z-10 w-full max-w-xl text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold text-white leading-[1.1] mb-5">
          The Whiteboard<br />
          <span className="text-teal-400">Image Prompt</span>
        </h1>

        {/* Subheadline */}
        <p className="text-gray-400 text-[17px] leading-[1.7] mb-6 max-w-md mx-auto">
          The exact prompt I use to generate photorealistic professor whiteboard images for X and LinkedIn — works with Grok, Nano Banana, and any image model that accepts a system prompt.
        </p>
        ...
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl px-6 py-5 mb-8 text-left">
          ...
          <div className="space-y-3">
            {[
              'The full system prompt — copy, paste, run',
              'iPhone 16 Pro photo realism with zero design skills',
              'Works for any topic — LLMs, frameworks, business concepts',
              'Stops the scroll on X and LinkedIn consistently',
              'Color hierarchy rules for academic-style readability',
            ].map((item) => (
        ...
        {status === 'success' ? (
          <div className="bg-teal-400/10 border border-teal-400/30 rounded-2xl px-8 py-8">
            ...
            <p className="text-gray-400 text-sm">
              Check your inbox — the prompt is heading to <span className="text-white">{email}</span> now.
            </p>
          </div>
        ) : (
